...
export default function Home() {
  ...
  const { posts } = useSelector((c: storeState) => c.Post);
...
setfavourites(storedFav)
  ...
  return (
    ...
    <Stack spacing={2} sx={{maxWidth:"1000px",paddingInline:4 , margin:"2rem auto"}}>
       <Createpost/>
      {posts.map((post:Post)=> <Postcard key={post._id} limitComment={1} post={post} favourites={favourites} handleTogle={handleTogle}  />)}
